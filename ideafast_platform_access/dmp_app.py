...
from .dmp_connection import DmpConnection, DmpLoginState
from .dmp_user_info import DmpUserInfo
from .dmp_utils import save_json_with_backup, FileTransaction
from .dmp_data_cache import DmpDataCache
from .dmp_file_db import DmpFileDb, DmpFileSet, DmpFileInfo
...
def _run_dmp_state():
    ...
    if cache_folder_name is None or cache_folder_name == '':
        ...
        print(f'                 {Fore.LIGHTYELLOW_EX}Please run "dmpapp configure <datafolder>"{Fore.RESET}')
    else:
    ...
    with DmpConnection('dmpapp') as dc:
        ...
        print(f'     {Fore.LIGHTBLACK_EX}State file: {login_state.appstate.state_file_name}{Fore.RESET}')
        if login_state.username is None:
            print(f'      User name: {Fore.LIGHTRED_EX}Not Specified{Fore.RESET}')
        else:
            print(f'      User name: {Fore.LIGHTGREEN_EX}{login_state.username}{Fore.RESET}')
        if not dc.is_logged_in:
            print(f'{Fore.LIGHTRED_EX}You are not logged in{Fore.RESET}')
        else:
            info = login_state.user_info
            print(f'           Name: {Fore.LIGHTGREEN_EX}{info.firstname or ""} {info.lastname or ""}{Fore.RESET}')
            print(f'          Email: {Fore.LIGHTGREEN_EX}{info.email or ""}{Fore.RESET}')
            print(f'Account Created: {Fore.LIGHTGREEN_EX}{info.created or ""}{Fore.RESET}')
            print(f'Account Expires: {Fore.LIGHTGREEN_EX}{info.expires or ""}{Fore.RESET}')
            ...
            if studies is not None:
                if len(studies) == 0:
                    print(f'{Fore.LIGHTRED_EX}You do not have access to any studies!{Fore.RESET}')
                else:
                    print(f'You have access to {len(studies)} studies:')
                    for study_id, study_name in studies.items():
                        print(f'  {Fore.LIGHTCYAN_EX}{study_id}{Fore.RESET} = "{Fore.YELLOW}{study_name}{Fore.RESET}"')
            pass
    pass
    ...
    with DmpConnection('dmpapp') as dc:
        ...
        if not dc.is_logged_in:
            ...
            if study_prefix is None or study_prefix == '':
                ...
                if len(ids) == 0:
                    print(f'{crd}No known study IDs matching "{ccy}{study_prefix}{crd}" found{c0}')
                elif len(ids) > 1:
                    print(f'{cyw}Prefix "{ccy}{study_prefix}{cyw}" is ambiguous.{c0} Matches:')
                    for stid in ids:
                        print(f'      {cor}{stid}{c0}')
                else:
...
def _run_dmp_login(username: Optional[str]):
    ...
    with DmpConnection('dmpapp') as dc:
        ...
        password = getpass(f'Password for user "{username}": ')  # cannot use color in that string
        if password == '':
...
def _run_dmp_files(study_id_keys: List[str]):
    ...
    if not cache.is_configured:
        print(f'{crd}No data folder configured. Run "dmpapp configure <your-data-folder>" first{c0}')
        exit(1)
        ...
        if not dc.is_logged_in:
            ...
            if studies is None:
                ...
                if study_id_keys is None or len(study_id_keys) == 0:
                    ...
                    if defstudy is None:
                        print(f'{crd}No study IDs provided and no default study selected{c0}')
                        print(f'Consider using {cgn}dmpapp study{c0} to establish a default study')
                        return
                ...
                for study_id in study_ids:
                    ...
                    print(f'Processing study "{cgn}{study_id}{c0}" ("{cbl}{study_name}{c0}"):')
...
def _get_filtered_list(
        ...
        fids: Optional[List[str]]) -> Optional[DataFrameGroupBy]:
    crd = Fore.LIGHTRED_EX
    cor = Fore.YELLOW
    c0 = Fore.RESET
    study0 = study
    if study is None or study == '':
        with DmpConnection('dmpapp') as dc:
            ...
            if study is None:
                print(f'{crd}No default study ID configured{c0} (use "dmpapp study" to do so)')
                return None
    ...
    renamed = selection.rename(columns={
        'fileId': 'file_id',
        'fileName': 'file_name',
        'fileSize': 'size',
    ...
    if len(groupkeys) == 0:
        # Just a trick to make everything work in case no filters/groups were given:
...
def _run_dmp_list(
    ...
    if n_id >= 0:
        ...
        combo = combo[['subject', 'kind', 'device_id', 't_from_utc', 'size', 'file_name', 'file_id_prefix']]
...
def _run_dmp_sync(
    ...
    with DmpConnection('dmpapp') as dc:
        ...
        selected_files = study_files.project_ids(DmpFileDb.extract_ids(combo, 'file_id'))
        ...
        for fpath, group in itertools.groupby(sorted_by_path, key=lambda x: str(x.path_name())):
            files = [dfi for dfi in group]
            if len(files) != 1:
                rejected_files[fpath] = files
                print(f'{crd}Skipping ambiguous files!{c0} ' +
        ...
        outdated_files.sort(key= lambda dfi: dfi.file_size)  # smallest files first

        print(f'Found {cgn}{len(outdated_files)}{c0} missing our outdated files.')
        remaining = 0
        if cap < len(outdated_files):
            print(f'More outdated files than cap! {crd}Capping download count to {cap}{c0}')
            remaining = len(outdated_files) - cap
            outdated_files = outdated_files[0:cap]
        print(f'Files selected for download ({cgn}{len(outdated_files)}{c0}, smallest files first):')
        for dfi in outdated_files:
            p = dfi.path_name()
            print(f'{cbl}{dfi.file_id}{c0}: {cor}{dfi.file_size:10}{c0} {cgn}{str(p)}{c0}')
        ...
        if len(outdated_files) == 0:
            ...
            for dfi in outdated_files:
                print(f'Downloading {cgn}{dfi.path_name()}{c0} ({cbl}{dfi.file_id}{c0})')

                def progress(sz: int) -> None:
                    percent = int(100 * sz / dfi.file_size)
                    print(f'\r{cyw}{sz:10}{c0} / {cbl}{dfi.file_size:10}{c0} / {cor}{percent:3}%{c0}  ', end='')
        ...
        if len(rejected_files) > 0:
            ...
            for fpath, files in rejected_files.items():
                print(f'  Skipped {crd}{len(files)}{c0} distinct files named {cor}{fpath}{c0}')

        if remaining > 0:
            print(f'{cbl}Some files were skipped because there were more than the download cap{c0}:')
            print(f'  There are {crd}{remaining}{c0} more files to download. Consider using the -cap option')
...
def _run_dmp_onefile(study: Optional[str], file_id: str, file_name: Optional[str]):
    ...
    with DmpConnection('dmpapp') as dc:
        ...
        if len(files) == 0:
            ...
            return
        if len(files) > 1:
            print(f'{crd}File prefix {cyw}{file_id}{crd} is ambiguous: {len(files)} matches{c0}')
            return
        dfi = files[0]
        print(f'Processing file {ccy}{dfi.file_id}{c0} / {cgn}{dfi.file_name}{c0} / {cyw}{dfi.file_size}{c0} bytes')
...
def run_dmp_app(*arguments: str):
    ...
    parser_state = subparsers.add_parser(
        'state',
        help='Print your current login state and accessible studies')

    parser_login = subparsers.add_parser(
        'login',
        help='Log the named user in (and log out the current user, if any)')
    ...
    parser_refresh = subparsers.add_parser(
        'refresh',
        help='Refresh your login information from the server')
    ...
    parser_sync.add_argument(
        '-p', nargs='+', type=str, dest='sync_p',
        help='Select one or more participants')
    parser_sync.add_argument(
        '-k', nargs='+', type=str, dest='sync_k',
        help='Select one or more device kinds (three letter codes)')
    parser_sync.add_argument(
        '-d', nargs='+', type=str, dest='sync_d',
        help='Select one or more devices (full ids)')
    parser_sync.add_argument(
        '-id', nargs='+', type=str, dest='sync_id',
        help='Select one or more file id prefixes')
    parser_sync.add_argument(
        '-cap', type=int, dest='sync_cap', default=1,
        help='Set the maximum number of files to download (default 1)')

    parser_onefile = subparsers.add_parser(
        'onefile',
        help='Download a single file')
    parser_onefile.add_argument(
        '-id', type=str, dest='onefile_id', required=True,
        help='The file id (or prefix)')
    parser_onefile.add_argument(
        '-out', type=str, dest='onefile_out',
    ...
    try:
        ...
        if cmd == 'state':
            ...
            if cap < 0:
                cap = 100000
            _run_dmp_sync(None, args.sync_p, args.sync_k, args.sync_d, args.sync_id, cap)
        elif cmd == 'onefile':
